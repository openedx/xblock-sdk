...
class ThreeThumbsTest(SeleniumTest):
    ...
    def test_three_thumbs_initial_state(self):
        ...
        link = self.browser.find_element_by_link_text(b'three thumbs test')
        link.click()
        self.wait_for_page_load(link, timeout=10)

        # The header reflects the XBlock
        header1 = self.browser.find_element_by_css_selector(b'h1')
        self.assertEqual(header1.text, b'XBlock: three thumbs test')

        # She sees that there are 3 sets of thumbs
        vertical_css = b'div.student_view > div.xblock-v1 > div.vertical'
        ...
        thumb_css = b'div.xblock-v1[data-block-type="thumbs"]'
        ...
        up_count_css = b'span.upvote span.count'
        down_count_css = b'span.downvote span.count'

        for thumb in thumbs:
            up_count = thumb.find_element_by_css_selector(up_count_css)
            down_count = thumb.find_element_by_css_selector(down_count_css)
            self.assertEqual(b'0', up_count.text)
            self.assertEqual(b'0', down_count.text)
    ...
    def test_three_upvoting(self):
        ...
        thumbs[0].find_element_by_css_selector(b'span.upvote').click()

        # Only the first thumb's upcount should increase
        _ = EmptyPromise(
            lambda: int(thumbs[0].find_element_by_css_selector(up_count_css).text) == 1,
            b"upvote action succeeded"
        ).fulfill()
        self.assertEqual(b'0', thumbs[1].find_element_by_css_selector(up_count_css).text)
        self.assertEqual(b'0', thumbs[2].find_element_by_css_selector(up_count_css).text)
    ...
    def test_three_downvoting(self):
        ...
        # Up vote for the first thumb
        thumbs[0].find_element_by_css_selector(b'span.downvote').click()

        # Only the first thumb's downcount should increase
        _ = EmptyPromise(
            lambda: int(thumbs[0].find_element_by_css_selector(down_count_css).text) == 1,
            b"downvote action succeeded"
        ).fulfill()
        self.assertEqual(b'0', thumbs[1].find_element_by_css_selector(down_count_css).text)
        self.assertEqual(b'0', thumbs[2].find_element_by_css_selector(down_count_css).text)
